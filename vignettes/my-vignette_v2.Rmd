...
We also have out-sample rolling forecast prediction function `predict.rolling` with following parameters. Specifically, using given model parameters `object`, start from the last time point $X_t$ in `data`, we obtain the one step ahead prediction $X_{t+1}$. Then fit the corresponding model using all available `data` and $X_{t+1}$ to obtain $X_{t+2}$. Repeat this process to $X_{t + n.head}$. 
...
We use the same set of data in tenAR model. Here we do a bit change in axis and specify a rank r. Notice that in the factor part, the dimension of input data xx is $d_1\times d_2\cdots d_k\times T$.
```{r eval=FALSE}
# Use the same simulated data x as in tenAR part
xx <- aperm(xx@data,c(2,3,4,1)) # Permute to d1*d2*d3*T
...
* `Ft` (Estimated factor over time), 
* `Ft.all` (Sum of Ft over time), 
* `Q` (Loading matrix), 
* `xhat` (The fitted series), 
* `norm.percent` (Percentage of residual of the model, in the form of Frobenius norm), 
* `niter` (Iterations used before stopping).

```{r, eval=FALSE}
Ft = model$Ft
Loading = model$Q
residual.percent = model$norm.percent
```
