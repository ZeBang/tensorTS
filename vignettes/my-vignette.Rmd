...
A <- generateA(dim, R=2) # randomly generate coefficient matrices with number of terms R = 1
xx <- generate(A, t=500) # generate tensor time series with AR(1) structure
```


Specify the estimation you want to use in function `TenAR` by parameter `method`, which can be
...
And specify you want to use multiple term model or one term model by parameter `r`, and the order in TenAR($p$) by parameter `p`, For example, if we want to estimate by `LSE` method under multiple term model with number of terms $r=2$ in TenAR(1) with $p=1$, run as follows,
...
```{r, eval=FALSE}
pred.xx <- predict.tenar(model, xx, n.head = 5, sd = TRUE)
```
