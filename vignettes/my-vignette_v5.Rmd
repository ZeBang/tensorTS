...
xx <- tenAR.sim(t=500, dim, R=2, P=1, rho=0.5, cov='iid') # generate tensor time series with AR(1) structure
```

To use estimation function `TenAR.est`, you need specify the estimation method you want to use by parameter `method`, which can be
...
Specify you want to use multiple term model or one term model by parameter `R`, and the order in TenAR($p$) by parameter `P`, For example, if we want to estimate by `LSE` method under multiple term model with number of terms $R=1$ in TenAR(1) with $P=1$, run as follows,

```{r, eval=FALSE}
model = tenAR.est(xx, R=1, P=1, method="LSE")
```
...
For predictions, we have function `predict.tenar` with following parameters. Specifically, using given model parameters `object`, start from the last time point $X_t$ in `data`, we obtain the one step ahead prediction $X_{t+1}$. Then again using given model parameters `object` to obtain $X_{t+2}$. Repeat this process to $X_{t + n.head}$. 


* `object` a fit from `TenAR.est`
...
```{r, eval=FALSE}
pred <- tenAR.predict(model, xx, n.head = 5)
```
...
xx <- tenAR.sim(t=500, dim, R=2, P=1, rho=0.5, cov='iid') # generate matrix time series with AR(1) structure
est <- matAR.RR.est(xx, method="RRLSE", k1=1, k2=1)
```

## Tensor Factor Models

The factor model part contains two main functions : A factor estimation function TenFM.est, and TenFM.rank for rank determination.
...
* The function returns factor.num, which is the estimated rank.

```{r eval=FALSE}
r.est = TenFM.rank(xx,r)
